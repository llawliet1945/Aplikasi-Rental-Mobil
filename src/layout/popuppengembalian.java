...
public class popuppengembalian extends javax.swing.JFrame {
    ...
    public invoice kmbl = null;
    public popuppengembalian() {
        initComponents();
        datatable();
    }
    protected void datatable(){
        Object[] row = {"Id Sewa","Id Mobil","Id Penyewa","Tanggal Sewa","Tanggal Kembali"};
        tabmode = new DefaultTableModel(null, row);
        
        try {
            String sql = "select * from pengembalian order by idsewa asc";
    ...
    private void tblSewaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblSewaMouseClicked
        ...
        try {
            kmbl.idsewa = tblSewa.getValueAt(tblnyewa, 0).toString();
            kmbl.idmobil = tblSewa.getValueAt(tblnyewa, 1).toString();
            kmbl.idpenyewa = tblSewa.getValueAt(tblnyewa, 2).toString();
            kmbl.tgl = Date.valueOf(tblSewa.getValueAt(tblnyewa, 3).toString());
            kmbl.tgl2 = Date.valueOf(tblSewa.getValueAt(tblnyewa, 4).toString());
            kmbl.kembaliTerpilih();
        } catch (Exception ex) {
            Logger.getLogger(popuppengembalian.class.getName()).log(Level.SEVERE, null, ex);
        }
        this.dispose();
